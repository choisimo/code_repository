...
find_files() {
    ...
    for fmt in "${file_formats[@]}"; do
        case_insensitive+=(-iname "$fmt")
    done
    
    find "${find_args[@]}" \( "${case_insensitive[@]}" \) -print0 2> >(grep -v 'Permission denied' >&2)
    ...
    done < <(find "$start_dir" -type d -print0 2>/dev/null)
...
enhanced_test() {
    ...
    # 오류 발생 시 종료되지 않도록 변경
    find_result=$(find_files 2>/dev/null || true)
    
    if [ -z "$find_result" ]; then
        echo "⚠️ 파일 검색 실패 또는 파일 없음"
        log_warning "파일 검색 실패 또는 일치하는 파일 없음"
    else
...
process_files() {
    ...
    trap 'log_error "파일 처리 중단"; exit 1' SIGTERM SIGINT
    while IFS= read -r -d $'\0' file; do
    ...
    done < <(find_files 2>/dev/null || true)  # find 오류 무시
}
